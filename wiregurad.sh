...
update_kernel(){
    ...
    yum  -y update
    yum  -y install firewalld net-tools curl ppp pptpd  xl2tp git wget dkms 
gcc-c++ gcc-gfortran glibc-headers glibc-devel libquadmath-devel libtool 
systemtap systemtap-devel  qrencode wget git bash-completion vim gedit 
make wireguard-dkms wireguard-tools kmod-wireguard libreswan lsof 
nspr-devel nss pam flex bison xmlto  pkgconfig libseccomp-dev libsystemd-dev 
iproute2 awk which Networkmanager elf2flt arm
...
firewall-cmd --add-port=323/tcp --permanent
...
echo "电脑端请下载client.conf，手机端可直接使用软件扫码+shell order-qrencode -o -l -t UTF8 vpscloudip"

echo "${content}" | qrencode -o -l -t UTF8  
